...
Circle* Circle::Parse(string stringLine) {
    string delimeter = "(), ";
    ...
    float R = tokenizer.nextDouble();

    if (!Circle::isValid(R)) return new Circle();
    Point * center = new Point(x1,x2);
    Circle * Cir = new Circle(center, R);
    return Cir;
}